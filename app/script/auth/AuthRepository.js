...
z.auth.AuthRepository = class AuthRepository {
   ...
   * @param {z.event.WebSocketService.CHANGE_TRIGGER} [websocket_trigger] - Trigger for websocket reset
   * @returns {undefined} No return value
   */
  renew_access_token(renewal_trigger, websocket_trigger) {
    ...
    if (!is_refreshing_token) {
      ...
      this.get_access_token()
        .then(() => {
          ...
          amplify.publish(z.event.WebApp.CONNECTION.ACCESS_TOKEN.RENEWED, websocket_trigger);
        })
        .catch((error) => {
